...
#Comando que envía kaomojis
  ...
  async def cripto(self, ctx: commands.Context):
    ...
    """
    block = "`"*3
    result = cg.get_price(ids='ethereum,tether,cardano,dogecoin,monero,basic-attention-token,shiba-inu,binancecoin,pancakeswap-token', vs_currencies='usd')
  ...
  async def randomcolor(self, ctx: commands.Context):
    ...
    em_color = discord.Embed(title = f"🎨 Color aleatorio elegido: {name}", color = int("0x"+hex))
