...
class Song:
    ...
    def create_embed(self):
        em = (
            ...
            .add_field(
                name="⏱️ Duración", value=f"`{self.source.duration}`", inline=True
            )
            .add_field(
            ...
            .set_footer(
                text=f"Solicitado por {self.requester.name} (In streaming)",
...
class music(commands.Cog):
    ...
    @commands.command(
        help="Hazme unir a un canal de voz.",
        name="join",
        aliases=["j"],
    ...
    async def _join(self, ctx: commands.Context):
        ...
        em = discord.Embed(
            title=f"✅ Me he conectado en {destination}", color=ctx.author.color
        )
    ...
    async def _summon(
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError(
                "No estás conectado a ningún canal de voz / no especificaste a que canal unirme."
            )
        ...
        if ctx.voice_state.voice:
            await ctx.voice_state.voice.move_to(destination)
            em = discord.Embed(
                title=f"✅ He sido invocada en {destination}", color=ctx.author.color
            )
            em.set_footer(text=f"Solicitado por {ctx.author.name}")
            await ctx.send(embed=em)
    ...
    @commands.command(
        help="Hazme salir de un canal de voz.", name="leave", aliases=["l"]
    )
    async def _leave(self, ctx: commands.Context):
        ...
        em = discord.Embed(
            title=f":zzz: Desconectada de {dest}", color=ctx.author.color
        )
        em.set_footer(text=f"Solicitado por {ctx.author.name}")
        await ctx.send(embed=em)
    ...
    async def syt(self, ctx, *, search):
        ...
        await ctx.send(
            "🔎 **Esto fue lo que encontré en Youtube. ¿Es lo que buscabas?** <a:neko_pls:846610049203568680>\n"
            + yt_search
        )

    @commands.command(
        help="Ajusta el volumen de mi reproducción.", name="volume", aliases=["vol"]
    )
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        em = discord.Embed(
            title=f"Volumen ajustado a **`{volume}%`**", color=ctx.author.color
        )
        em.set_footer(text=f"Solicitado por {ctx.author.name}")
        await ctx.send(embed=em)

    @commands.command(
        help="Mira que se está reproduciendo actualmente.",
        name="now",
        aliases=["n", "current", "playing"],
    )
    ...
    @commands.command(
        name="resume", help="Reanuda la reproducción pausada.", aliases=["r"]
    )
    ...
    @commands.command(
        name="stop", help="Detén la lista de reproducción.", aliases=["st"]
    )
    async def _stop(self, ctx):
        ...
        em = discord.Embed(
            title=f"🛑 Vale, ya no reproduciré nada más.", color=ctx.author.color
        )
        em.set_footer(
    ...
    @commands.command(
        name="queue", help="Mira la lista de reproducción actual.", aliases=["q"]
    )
    ...
    @commands.command(
        name="shuffle", help="Aleatoriza la lista de reproducción.", aliases=["sh"]
    )
    ...
    @commands.command(
        name="remove", help="Remueve una canción de la lista", aliases=["re"]
    )
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send(
                    ":headphones: Agregado a la lista {}".format(str(source))
                )
